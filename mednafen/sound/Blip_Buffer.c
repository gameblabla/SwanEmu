...
blargg_err_t Blip_Buffer_set_sample_rate(Blip_Buffer* bbuf, long new_rate, int msec)
{
...
{
	assert(count <= Blip_Buffer_samples_avail(bbuf));   // tried to remove more samples than available
...
void Blip_Buffer_remove_samples(Blip_Buffer* bbuf, long count)
{
	if (count)
	{
		...
		memmove(bbuf->buffer, bbuf->buffer + count, remain * sizeof(blip_buf_t_));
		memset(bbuf->buffer + remain, 0, count * sizeof(blip_buf_t_));
	}
}
...
{
	blip_buf_t_* out = bbuf->buffer + (bbuf->offset >> BLIP_BUFFER_ACCURACY) + blip_widest_impulse_ / 2;
